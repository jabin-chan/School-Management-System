...
async function updateStatus(req, res) {
  const { id } = req.params;
  const { status, student_id, password } = req.body;
  ...
  if (status === 'passed') {
    ...
    if (student_id) {
      const taken = await studentModel.findByStudentId(student_id);
      if (taken) throw createError(409, 'Student ID already exists');
    }

    const currentSession = await sessionModel.findCurrent();
    application.session_id = currentSession ? currentSession.session_id : null;

    const created = await studentModel.createFromApplication(application, { student_id, password });
