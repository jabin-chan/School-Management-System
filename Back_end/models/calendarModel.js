...
async function findById(id) {
  const [rows] = await pool.query('SELECT * FROM academic_calendar WHERE event_id = ? LIMIT 1', [id]);
  return rows[0] || null;
}
...
async function update(id, data) {
  ...
  values.push(id);
  const [result] = await pool.query(`UPDATE academic_calendar SET ${updates.join(', ')} WHERE event_id = ?`, values);
  return result.affectedRows > 0;
}

async function remove(id) {
  const [result] = await pool.query('DELETE FROM academic_calendar WHERE event_id = ?', [id]);
  return result.affectedRows > 0;
}
