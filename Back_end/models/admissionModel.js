...
async function findAll({ status, page = 1, limit = 20 }) {
  ...
  const [rows] = await pool.query(
    `SELECT a.*, st.id AS linked_student_id, st.student_id AS linked_student_student_id
     ...
     LEFT JOIN students st ON st.application_id = a.application_id
     ${where}
...
async function findById(id) {
  const [rows] = await pool.query('SELECT * FROM admission_applications WHERE application_id = ? LIMIT 1', [id]);
  return rows[0] || null;
}

async function updateStatus(id, status) {
  const [result] = await pool.query('UPDATE admission_applications SET status = ? WHERE application_id = ?', [status, id]);
