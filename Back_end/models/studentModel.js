...
async function createFromApplication(application, options = {}) {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();

    const studentId = options.student_id || await generateUniqueStudentId(connection);
    const rollNumber = await nextRollNumber(connection, application.class);
    const rawPassword = options.password || crypto.randomBytes(6).toString('hex');
    ...
    await connection.query('UPDATE admission_applications SET status = ? WHERE application_id = ?', ['passed', application.id]);
...
async function findByStudentId(studentId) {
  const [rows] = await pool.query(
    `SELECT ${PUBLIC_FIELDS} FROM students WHERE student_id = ? LIMIT 1`,
    [studentId]
  );
  return rows[0] || null;
}
