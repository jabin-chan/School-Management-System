...
async function generateUniqueTeacherId() {
  for (let i = 0; i < 10; i += 1) {
    ...
    const [rows] = await pool.query('SELECT id FROM teachers WHERE teacher_id = ? LIMIT 1', [candidate]);
...
async function findById(id) {
  const [rows] = await pool.query(`SELECT ${FIELDS.join(', ')} FROM teachers WHERE id = ? LIMIT 1`, [id]);
  return rows[0] || null;
}
...
async function update(id, data) {
  ...
  values.push(id);
  const [result] = await pool.query(`UPDATE teachers SET ${updates.join(', ')} WHERE id = ?`, values);
