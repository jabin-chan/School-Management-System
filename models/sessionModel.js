...
async function findById(id) {
  const [rows] = await pool.query('SELECT * FROM academic_sessions WHERE id = ? LIMIT 1', [id]);
  return rows[0] || null;
}
...
async function update(id, data) {
  ...
  values.push(id);
  const [result] = await pool.query(`UPDATE academic_sessions SET ${updates.join(', ')} WHERE id = ?`, values);
...
async function setCurrent(id) {
  ...
  try {
    ...
    const [result] = await connection.query('UPDATE academic_sessions SET is_current = 1 WHERE id = ?', [id]);
...
async function remove(id) {
  const [result] = await pool.query('DELETE FROM academic_sessions WHERE id = ?', [id]);
  return result.affectedRows > 0;
}
